	...
	while(FOREVER())
	{
		...
		{       
			...
			if(retry_count >= 2)
			{
				...
				WalError("sync retry notification has reached max attempts, proceeding even though cloud and CPE are out of sync\n");
			}
		}
		else
		{
        ...
        if (dest != NULL)
        {
	        ...
	        switch (notifyData->type)
	        {
				...
				if(cmc == 768)
				{
					...
					{
						WalError("Failed to generate unformatted JSON string from 'notification' using cJSON_PrintUnformatted.\n");
					}
