...
static int contructRollbackTableData(parameterValStruct_t **parameterval,int paramCount,char ***rowList,int rowCount, int *numParam,TableData ** getList, money_trace_spans *timeSpan);
static int getWritableParams(char *paramName, char ***writableParams, int *paramCount, money_trace_spans *timeSpan);
static int cacheTableData(char *objectName,int paramcount,char ***rowList,int *numRows,int *params,TableData ** list, money_trace_spans *timeSpan);
static int addNewData(char *objectName,TableData * list,int paramcount, money_trace_spans *timeSpan);
static void deleteAllTableData(char **deleteList,int rowCount, money_trace_spans *timeSpan);
static void addCachedData(char *objectName,TableData * addList,int rowCount, money_trace_spans *timeSpan);
...
void replaceTable(char *objectName,TableData * list,unsigned int paramcount, money_trace_spans *timeSpan, WDMP_STATUS *retStatus)
{
    ...
    else
    {
        ...
        ret = cacheTableData(paramName,paramcount,&deleteList,&rowCount,&numParams,&addList, timeSpan);
        WalPrint("ret : %d rowCount %d numParams: %d\n",ret,rowCount,numParams);
        if(ret == CCSP_SUCCESS)
        {
            WalInfo("Table (%s) has %d rows\n",paramName,rowCount);
            if(rowCount > 0)
            {
            ...
            deleteAllTableData(deleteList,rowCount, timeSpan);
            if(paramcount != 0)
            {
                addRet = addNewData(objectName,list,paramcount, timeSpan);
                ret = addRet;
                isWalStatus = 1;
                if(addRet != WDMP_SUCCESS && rowCount > 0)
                {
                    WalError("Failed to replace table, hence reverting the changes\n");
                    addCachedData(objectName,addList,rowCount, timeSpan);
                }
                if(addList != NULL)
                {
                    for ( cnt = 0 ; cnt < rowCount ; cnt++)
...
static int cacheTableData(char *objectName,int paramcount,char ***rowList,int *numRows,int *params,TableData ** list, money_trace_spans *timeSpan)
{
    ...
    if (ret == CCSP_SUCCESS && size == 1)
    {
        ...
        if(timeSpan)
        {
            ...
            timeSpan->spans[0].name = strdup(ppComponents[0]->componentName);
        ...
        if(ret == CCSP_SUCCESS && val_size > 0)
        {
            ...
            *numRows = rowCount;
            ...
            {
                timeDuration = 0;
                ret = contructRollbackTableData(parameterval,val_size,rowList,rowCount,params,&getList, timeSpan);
                if(ret == CCSP_SUCCESS)
                {
                    *list = getList;
                    for(cnt =0; cnt < rowCount; cnt++)
...
{
    int cnt =0, delRet = 0;
    money_trace_spans *deleteRowSpan = NULL;
    WalPrint("---------- Start of deleteAllTableData -----------\n");
    for(cnt =0; cnt < rowCount; cnt++)
    {
        if(timeSpan)
        {
            deleteRowSpan = (money_trace_spans *) malloc(sizeof(money_trace_spans));
            memset(deleteRowSpan,0,(sizeof(money_trace_spans)));
        }
        delRet = deleteRow(deleteList[cnt], deleteRowSpan);
        if(timeSpan)
        {
            if(deleteRowSpan->count > 0)
            {
                WalPrint("deleteRowSpan->spans[0].duration : %d\n", deleteRowSpan->spans[0].duration);
                timeSpan->spans[0].duration += deleteRowSpan->spans[0].duration;
                WalPrint("timeSpan->spans[0].duration = %d\n", timeSpan->spans[0].duration);
                WAL_FREE(deleteRowSpan->spans[0].name);
                WAL_FREE(deleteRowSpan->spans);
            }
            WAL_FREE(deleteRowSpan);
        }
        WalPrint("delRet: %d\n",delRet);
...
static int addNewData(char *objectName,TableData * list,int paramcount, money_trace_spans *timeSpan)
{
    ...
    money_trace_spans *addRowSpan = NULL, *deleteRowSpan = NULL;
    ...
    {				
        ...
        if(timeSpan)
        {
            addRowSpan = (money_trace_spans *) malloc(sizeof(money_trace_spans));
            memset(addRowSpan,0,(sizeof(money_trace_spans)));
        }
        addRowTable(paramName,&list[cnt],&retObject[cnt], addRowSpan, &addRet);
        if(timeSpan)
        {
            if(addRowSpan->count > 0)
            {
                WalPrint("addRowSpan->spans[0].duration : %d\n", addRowSpan->spans[0].duration);
                timeSpan->spans[0].duration += addRowSpan->spans[0].duration;
                WalPrint("timeSpan->spans[0].duration = %d\n", timeSpan->spans[0].duration);
                WAL_FREE(addRowSpan->spans[0].name);
                WAL_FREE(addRowSpan->spans);
            }
            WAL_FREE(addRowSpan);
        }
        WalPrint("addRet : %d\n",addRet);
        if(addRet != WDMP_SUCCESS)
        {
            ...
            for(i= cnt-1; i >= 0; i--)
            {
                ...
                deleteRowTable(paramName, deleteRowSpan, &delRet);
                if(timeSpan)
                {
                    if(deleteRowSpan->count > 0)
                    {
...
static void addCachedData(char *objectName,TableData * addList,int rowCount, money_trace_spans *timeSpan)
{
    ...
    money_trace_spans *addRowSpan = NULL;
    ...
    for(cnt =0; cnt < rowCount; cnt++)
    {
        ...
        addRowTable(paramName,&addList[cnt],&retRows[cnt], addRowSpan, &addRet);
        if(timeSpan)
        {
            if(addRowSpan->count > 0)
            {
...
static void getTableRows(char *objectName,parameterValStruct_t **parameterval, int paramCount, int *numRows,char ***rowObjects)
{
    ...
    for (cnt = 0; cnt < paramCount; cnt++)
    {
        WalPrint("parameterval[%d]->parameterName : %s\n", cnt, parameterval[cnt]->parameterName);
...
static int contructRollbackTableData(parameterValStruct_t **parameterval,int paramCount,char ***rowList,int rowCount, int *numParam,TableData ** getList, money_trace_spans *timeSpan)
{
    ...
    ret = getWritableParams((*rowList[0]), &writableList, &writableParamCount, timeSpan);
