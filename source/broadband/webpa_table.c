...
static int addRow(char *object,char *compName,char *dbusPath, money_trace_spans *timeSpan,int *retIndex);
static int updateRow(char *objectName,TableData *list,char *compName,char *dbusPath, uint32_t *duration);
...
static int addNewData(char *objectName,TableData * list,int paramcount)
{
    ...
    {				
        ...
        addRowTable(paramName,&list[cnt],&retObject[cnt], NULL, &addRet);
        WalPrint("addRet : %d\n",addRet);
...
static void addCachedData(char *objectName,TableData * addList,int rowCount)
{
    ...
    for(cnt =0; cnt < rowCount; cnt++)
    {
        ...
        strncpy(paramName,objectName,sizeof(paramName));
        addRowTable(paramName,&addList[cnt],&retRows[cnt], NULL, &addRet);
        WalPrint("addRet : %d\n",addRet);
        if(addRet == WDMP_SUCCESS)
        {
